...
export const requestMomoPayment = (reservationId, amount, reservation_code) => async dispatch => {
    try {
      const response = await http.post('http://localhost:3307/api/public/payment', {
        reservationId,
        amount,
        reservation_code 
      });
...
export const requestMomoPayUrl = (reservationId, amount) => async dispatch => {
    try {
        const response = await http.post('http://localhost:3307/api/public/payment/get_pay_url', {
        reservationId,
        amount,
        });
        return response.data; // Trả về dữ liệu để xử lý tiếp
...
export const requestMomoPaymentBalance = (reservationId, amount) => async dispatch => {
    try {
      const response = await http.post('http://localhost:3307/api/public/payment/pay_balance', {
        reservationId,
        amount 
      });
      return response.data; // Trả về dữ liệu để xử lý tiếp
