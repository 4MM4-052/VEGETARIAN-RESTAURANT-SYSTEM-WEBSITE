...
const filteredReviews = useMemo(() => {
  // Đảm bảo reviewList không rỗng
  const list = reviewList || [];
  return filterStar === 0
    ? list
    : list.filter(r => Number(r.star) === filterStar);
}, [reviewList, filterStar]); // Khi reviewList thay đổi, filteredReviews SẼ tính toán lại
...
const stableModelConfig = useMemo(
  () => modelConfig,
  [modelConfig.level1, modelConfig.level2a, modelConfig.level2b]
);

useEffect(() => {
  if (!useModel) {
    // load dữ liệu thường (không AI)
    axios.get("http://localhost:8000/reviews")
      .then(res => {
        ...
        setStats(newStats);
        setReviewList([]); // không show explain khi không dùng model
      })
  ...
  // nếu bật model nhưng chưa upload file → chưa làm gì
  setIsLoading(false);
...
const handleUploadFile = async () => {
  ...
  formData.append("level1", modelConfig.level1);
  formData.append("level2a", modelConfig.level2a);
  formData.append("level2b", modelConfig.level2b);

  try {
    const res = await axios.post("http://localhost:8000/admin/reviews/upload-and-predict", formData);
    
    const reviewsWithId = res.data.reviews.map((r, index) => ({
      ...r,
      // Đảm bảo id luôn tồn tại và đồng nhất kiểu dữ liệu
    ...
    setReviewList(reviewsWithId);
    setStats(res.data.stats);
  } catch (err) { 
    console.error("Lỗi Upload:", err); 
  } finally {
    setUploading(false); // Đừng quên tắt loading nhé
  }
};
...
{useModel && (
  <div style={{
    ...
    <div style={{ fontSize: 14, fontWeight: 700, marginBottom: 8 }}>
      Upload dữ liệu đánh giá (CSV / Excel)
    </div>
    ...
    >
      {uploading ? 'Đang xử lý...' : 'Upload & dự đoán'}
    </button>
  </div>
)}
...
<div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
  ...
  >
    Đề xuất cải thiện
            ...
            {/* Thêm dòng này để kiểm tra dữ liệu khi render */}
{console.log("Đang render review:", r.id, "Phrases:", r.phrases)}
