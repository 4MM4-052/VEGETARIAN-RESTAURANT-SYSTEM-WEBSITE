...
//         const matchProduct = results.find(p => {
//             const productSlug = unidecode(p.name)
//                 .toLowerCase()
//                 .replace(/[^a-z0-9]/g, "-")
//                 .replace(/-+/g, "-")
//                 .replace(/^-+/, "")
...
//         matchProduct.image = `/menu/${matchProduct.image}`;
...
// // Thêm nhiều sản phẩm từ Excel
// router.post('/bulk', (req, res) => {
//     const { products } = req.body; // products là mảng từ frontend
...
//         if (err) {
//             console.error('Error inserting bulk products:', err);
//             return res.status(500).json({ error: 'Failed to add products' });
//         }
//         res.status(201).json({ message: 'Bulk products added successfully', insertedCount: results.affectedRows });
//     });
// });
...
router.get('/slug/:slug', (req, res) => {
    const { slug } = req.params;
    const decodedSlug = decodeURIComponent(slug).replace(/\.html$/, '').toLowerCase();
    ...
    connection.query(sql, [], (err, results) => {
        ...
        // Map ảnh: đảm bảo luôn có /menu/ ở đầu nếu chưa có
        if (matchProduct.image) {
            matchProduct.image = matchProduct.image.startsWith('/menu/')
                ? matchProduct.image
                : `${matchProduct.image}`;
        } else {
            // Nếu rỗng thì trả về chuỗi rỗng hoặc ảnh placeholder
            matchProduct.image = '';
        }
