...
router.post('/send', (req, res) => {
    ...
    // Thêm các món mới vào bảng changedishes
    ...
    Promise.all(insertQueries)
        .then(() => {
            const mailOptionsCustomer = {
                from: `"Hương Sen – Tịnh Chay Cho Mọi Hành Trình Tâm Thức" <${process.env.EMAIL_USERNAME}>`,
                to: customerInfo.email,
                subject: '[No-reply] - Yêu cầu thay đổi món ăn - Hương Sen – Tịnh Chay Cho Mọi Hành Trình Tâm Thức',
                html: `
                    <h3>Xin chào ${customerInfo.fullname},</h3>
                    <p>Bạn đã gửi yêu cầu thay đổi món ăn tại nhà hàng Hương Sen vào ngày ${formattedDateTime}. Dưới đây là thông tin chi tiết:</p>
                    <h4>Thông tin khách hàng:</h4>
        ...
        .catch((insertError) => {
            console.error('Lỗi khi thêm món vào changedishes:', insertError);
            return res.status(500).json({ status: 500, message: 'Lỗi khi thêm món vào changedishes' });
        });
});
