...
let sql = `
  SELECT 
    ...
    IFNULL(
      CONCAT('[', GROUP_CONCAT(
        CONCAT(
          '{"product_id":"', IFNULL(cd.product_id, ''), '",',
          '"quantity":"', IFNULL(cd.quantity, 0), '",',
          '"price":"', IFNULL(cd.price, 0), '",',
          '"total_amount":"', IFNULL(cd.total_amount, 0), '",',
          '"productName":"', IFNULL(cd.productName, ''), '",',
          '"productImage":"', IFNULL(cd.productImage, ''), '",',
          '"taxMoney":"', IFNULL(cd.taxMoney, 0), '",',
          '"reducedMoney":"', IFNULL(cd.reducedMoney, 0), '"}'
        )
      ), ']'),
      '[]'
    ) AS changedishes
...
`;
  ...
  let queryParams = [seaName, seaPhone, seaEmail, seaStatus, seaCode];
  if (page && limit) {
      sql += ' LIMIT ? OFFSET ?';
...
router.post("/", (req, res) => {
  ...
  connection.beginTransaction((err) => {
    ...
    findAvailableTable(reservation_date, partySize, (err, suitableTableId) => {
      if (err) {
        return rollbackTransaction(res, "Lỗi khi tìm bàn", err);
      }

      // Kiểm tra xem có bàn phù hợp không
      if (!suitableTableId) {
        return rollbackTransaction(res, "Không có bàn trống", null);
      }

      // Chỉ thêm đơn đặt bàn
      ...
      connection.query(
        ...
        [
          ...
          notes,
          totalAmount,
          suitableTableId, // Sử dụng ID bàn tìm được
        ],
        (err, results) => {
          if (err) {
